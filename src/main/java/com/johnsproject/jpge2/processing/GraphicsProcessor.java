...
public class GraphicsProcessor {
		...
		for (int i = 0; i < scene.getCameras().size(); i++) {
			...
			for (int j = 0; j < scene.getModels().size(); j++) {
				...
				for (int k = 0; k < scene.getLights().size(); k++) {
					...
					long[][] worldMatrix = model.getModelMatrix();
					long[][] normalMatrix = model.getNormalMatrix();
					long[][] viewMatrix = camera.getViewMatrix();
					long[][] projectionMatrix = camera.getPerspectiveMatrix();
					...
					for (int l = 0; l < model.getVertices().length; l++) {
						...
						vertex.reset();
						long[] location = vertex.getLocation();
						long[] normal = vertex.getNormal();
					...
					for (int l = 0; l < model.getFaces().length; l++) {
						...
						face.reset();
						long[] location1 = face.getVertex1().getLocation();
						long[] location2 = face.getVertex2().getLocation();
						long[] location3 = face.getVertex3().getLocation();
	...
	public static long[][] worldMatrix(long[][] matrix, Model model) {
		Transform transform = model.getTransform();
		long[] location = transform.getLocation();
		long[] rotation = transform.getRotation();
		long[] scale = transform.getScale();
	...
	public static long[][] normalMatrix(long[][] matrix, Model model) {
		Transform transform = model.getTransform();
		long[] rotation = transform.getRotation();
		long[] scale = transform.getScale();
		MatrixProcessor.reset(matrix);
	...
	public static long[][] viewMatrix(long[][] matrix, Camera camera) {
	...
	public static long[][] orthographicMatrix(long[][] matrix, Camera camera) {
		MatrixProcessor.reset(matrix);
		matrix[0][0] = -1;
		matrix[1][1] = -1;
		matrix[2][2] = 0;
	...
	public static long[][] perspectiveMatrix(long[][] matrix, Camera camera) {
		long[] frustum = camera.getViewFrustum();
		MatrixProcessor.reset(matrix);
	...
	private static final long[] pixel = VectorProcessor.generate();
	private static final long[] barycentric = VectorProcessor.generate();
	private static final long[] depth = VectorProcessor.generate();
	...
	public static void drawFace(Face face, GraphicsBuffer graphicsBuffer) {
		...
		int minX = (int)Math.min(location1[vx], Math.min(location2[vx], location3[vx]));
		int minY = (int)Math.min(location1[vy], Math.min(location2[vy], location3[vy]));
		int maxX = (int)Math.max(location1[vx], Math.max(location2[vx], location3[vx]));
		int maxY = (int)Math.max(location1[vy], Math.max(location2[vy], location3[vy]));
		...
		for (pixel[vy] = minY; pixel[vy] < maxY; pixel[vy]++) {
			for (pixel[vx] = minX; pixel[vx] < maxX; pixel[vx]++) {
				...
				if ((barycentric[vx] >= 0) && (barycentric[vy] >= 0) && (barycentric[vz] >= 0)) {
					pixel[vz] = interpolateDepth(depth, barycentric);
					int color = face.getMaterial().getShader().fragment(barycentric);
					graphicsBuffer.setPixel((int)pixel[vx], (int)pixel[vy], (int)pixel[vz], color);
				}
			}
		}
	}

	private static int interpolateDepth(long[] values, long[] barycentric) {
	...
	public static int interpolate(long[] values, long[] barycentric) {
	...
	private static long barycentric(long[] vector1, long[] vector2, long[] vector3) {
	...
	public static abstract class Shader {
		...
		public abstract int fragment(long[] barycentric);
	}
}
