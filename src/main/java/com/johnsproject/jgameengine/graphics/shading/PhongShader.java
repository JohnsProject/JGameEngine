...
import com.johnsproject.jgameengine.graphics.rasterization.LinearRasterizer6;
...
public class PhongShader extends ThreadedShader {
	...
	private static class GeometryShader extends ThreadedGeometryShader {
		...
		private int calculateLights(int[] location, int[] normal, Material material) {
			...
			for (int i = 0; i < lights.size(); i++) {
				...
				if(!light.isActive() || light.isCulled())
					continue;
				final int ambient = Color.multiplyColor(texelColor, light.getAmbientColor());
		...
		private int calculateDiffuseColor(int[] normal, Material material, Light light) {
			...
			int diffuse = Color.multiplyColor(material.getDiffuseColor(), texelColor);
			diffuse = Color.multiply(diffuse, diffuseIntesity);
		...
		private int calculateSpecularColor(int[] location, int[] normal, Material material, Light light) {
			...
			int specular = Color.multiplyColor(material.getSpecularColor(), texelColor);
			specular = Color.multiply(specular, specularIntensity);
