...
import com.johnsproject.jgameengine.math.FrustumType;
...
public class ForwardShaderBuffer implements ShaderBuffer {
	...
	public void initialize(Camera camera, List<Light> lights) {
		...
		for(int i = 0; i < lights.size(); i++) {
			...
			final long lightDistance = Vector.squaredDistance(lightLocation, cameraLocation);
	...
	private void initializeDirectionalLightMatrix() {
		if (shadowDirectionalLight != null) {
			...
			Matrix.copy(projectionMatrix, frustumProjectionMatrix);
			Matrix.multiply(projectionMatrix, lightSpaceMatrix, frustumProjectionMatrix);
		}
	}
