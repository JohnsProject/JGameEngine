...
public class PerspectivePhongRasterizer extends AffinePhongRasterizer {
	...
	private void drawScanline(int x1, int x2, int y, int z, int u, int v, int wx, int wy, int wz, int nx, int ny, int nz,
		...
		for (; x1 <= x2; x1++) {
            ...
            fragment.getWorldLocation()[VECTOR_X] = FixedPointMath.multiply(wx, oneByZ);
            fragment.getWorldLocation()[VECTOR_Y] = FixedPointMath.multiply(wy, oneByZ);
            fragment.getWorldLocation()[VECTOR_Z] = FixedPointMath.multiply(wz, oneByZ);
            fragment.getWorldNormal()[VECTOR_X] = FixedPointMath.multiply(nx, oneByZ);
            fragment.getWorldNormal()[VECTOR_Y] = FixedPointMath.multiply(ny, oneByZ);
            fragment.getWorldNormal()[VECTOR_Z] = FixedPointMath.multiply(nz, oneByZ);
