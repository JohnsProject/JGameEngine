...
import static com.johnsproject.jgameengine.math.Matrix.MATRIX_SIZE;
...
public class MatrixTest {
	...
	public void matrixValueOperationsTest() throws Exception {
		...
		for (int i = 1; i < 256; i++) {
			...
			// matrix subtract
			Matrix.subtract(fpMatrix1, fpValue1);
			subtract(matrix1, value1);
			assertMatrix(matrix1, toMatrix(fpMatrix1), precision);
			// matrix multiply
			Matrix.multiply(fpMatrix1, fpValue1);
			multiply(matrix1, value1);
			assertMatrix(matrix1, toMatrix(fpMatrix1), precision);
			// matrix divide
			Matrix.divide(fpMatrix1, fpValue1);
	...
	public void matrixMatrixOperationsTest() throws Exception {
		...
		for (int i = 1; i < 128; i++) {
			...
			// fill matrix
			Matrix.add(fpMatrix1, fpValue);
			Matrix.add(fpMatrix2, fpValue);
			double[][] matrix1 = toMatrix(fpMatrix1);
			double[][] matrix2 = toMatrix(fpMatrix2);
			double[][] result = toMatrix(fpResult);
			// matrix add
			Matrix.add(fpMatrix1, fpMatrix2, fpResult);
			result = add(matrix1, matrix2);
			assertMatrix(result, toMatrix(fpResult), precision);
			// matrix subtract
			Matrix.subtract(fpMatrix1, fpMatrix2, fpResult);
			result = subtract(matrix1, matrix2);
			assertMatrix(result, toMatrix(fpResult), precision);
			// matrix multiply
			Matrix.multiply(fpMatrix1, fpMatrix2, fpResult);
			result = multiply(matrix1, matrix2);
			assertMatrix(result, toMatrix(fpResult), precision);
			// matrix divide
			Matrix.divide(fpMatrix1, fpMatrix2, fpResult);
	...
	public void inverseTest() throws Exception {
		for (int i = 1; i < 16; i++) {
			...
			double[][] matrix1 = toMatrix(fpMatrix1);
			int[][] fpResult = Matrix.indentityMatrix();
			Matrix.inverse(fpMatrix1, fpResult);
